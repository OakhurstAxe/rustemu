...
pub mod vcs {
    ...
    use crate::vcs_console::vcs::{ VcsConsole, Message, VcsAudioEvent };
    ...
    impl VcsSdlMain {
        ...
        pub fn vcs_sdl_main(&mut self, rom_file: &str) -> Result<(), String> {
            ...
            // VCS Console
            let vcs_console: Arc<Mutex<VcsConsole>> = Arc::new(Mutex::new(VcsConsole::new(rom_file, event_sender)));
            ...
            let frames_per_second = vcs_console.lock().unwrap().get_console_type().read().unwrap().get_frames_per_second();
