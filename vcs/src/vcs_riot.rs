...
pub mod vcs {
    ...
    impl VcsRiot {
        ...
        pub fn new() -> Self {
            Self {
                system_ram: MemoryRam::new(String::from("RIOT Ram"), 0x80),
                step: 1,
                step_count: 0,
                overflow_tick: false,
        ...
        pub fn reset(&mut self) {
            self.step = 1;
            self.system_ram.write(REG_INSTAT, 0);
            self.system_ram.write(REG_SWCHA, 255);
            self.system_ram.write(REG_SWCNT, 255);
            self.system_ram.write(REG_SWBCNT, 255);
        ...
        pub fn execute_tick(&mut self) {
            ...
            let timer: u8 = self.system_ram.read(REG_INTIM);
            let (timer, _overflow) = timer.overflowing_sub(1);
            if timer == 0xFF {
                let mut status_byte: u8 = self.system_ram.read(REG_INSTAT);
                status_byte = status_byte | 0xFF;
                self.system_ram.write(REG_INSTAT, status_byte);
                self.step = 1;
                self.overflow_tick = true;
            }            
            self.system_ram.write(REG_INTIM, timer);
        }
        ...
        pub fn read(&mut self, location: u16) -> u8 {
            ...
            if location == REG_SWCHB {
                ...
                result = self.system_ram.read(location);
            }
        ...
        pub fn write(&mut self, location: u16, byte: u8) {
            ...
            if location == REG_TIMI1T {
                self.clear_timnnt_underflow();
                self.system_ram.write(REG_INTIM, byte_minus_1);
                self.step = 1;
                self.step_count = 0;
            }
